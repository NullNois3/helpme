...
module "resourceLocation" {
  ...
  constraint       = "gcp.resourceLocations"    # constraint identifier without constriants/ prefix. Example "compute.requireOsLogin"
  policy_type      = "list"            # either of list or boolean
  ...
  rules = [
    # Rule 1
    {
      enforcement = false
      allow       = []
      deny        = ["in:asia-east1-locations", "in:asia-east2-locations"]
      conditions  = [{
        description = "exempt via tag disable"
        expression  = "!resource.matchTag('/disable', 'domain-restricted-sharing')"
        location    = "sample-location.log"
